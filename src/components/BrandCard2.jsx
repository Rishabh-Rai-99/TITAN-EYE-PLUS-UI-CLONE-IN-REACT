  ...
  return (
    brandtwo.map((brands)=>{
        return <div key={brands.id} className="card relative flex justify-center">
          <div style={{background:`url(${brands.brand_img})`,backgroundPosition:"center"}} className='w-52 h-58 bg-contain rounded-xl bg-center'>
          </div>
            <div className='bg-[#EFFCFE] rounded-xl flex justify-center items-center absolute -bottom-10 w-46 h-22 mx-auto'>
            <img className='mix-blend-multiply' src={brands.brand_logo} alt="" />
            </div>
        </div>
    })
  )
